...
export class StatusCommand extends CommandRunner {
  ...
  async run([runId]: string[]): Promise<void> {
    ...
    this.write([
      ...
      ...run.steps.map((step) => `${step.id}: ${step.status}${step.kind === 'agent' && step.conditionDecision ? ' (condition false)' : ''}${step.kind === 'agent' && step.agentOverride ? ` (fallback: ${step.agentOverride.profile})` : ''}${step.kind === 'agent' && step.executionAuthorization === 'pre-authorized' ? ' (execution: pre-authorized)' : ''}`),
      ...failedOutputs,
